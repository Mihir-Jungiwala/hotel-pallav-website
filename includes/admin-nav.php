...
{
    // Needs-attention total: pending bookings + enquiries not yet resolved (new intake + pending).
    $pendingCount = (int) (db_one("SELECT COUNT(*) c FROM bookings WHERE status='pending'")['c'] ?? 0);
    $unreadCount = (int) (db_one("SELECT COUNT(*) c FROM enquiries WHERE status IN ('new','pending')")['c'] ?? 0);
    ...
    return [
        ...
        'Guest Activity' => [
            ['href' => 'admin/bookings.php', 'match' => 'bookings.php', 'label' => 'Guest Activity', 'icon' => '<rect x="3.5" y="5" width="17" height="15" rx="2.5"/><path d="M3.5 10h17M8.5 3v3.4M15.5 3v3.4"/>', 'badge' => $pendingCount + $unreadCount],
        ],
        'Homepage Content' => [
        ...
        'Site Settings' => [
            ...
            ['href' => 'admin/email-templates.php', 'match' => 'email-templates.php', 'label' => 'Email Templates', 'icon' => '<rect x="3" y="5" width="18" height="14" rx="2.5"/><path d="M3.6 7l8.4 6 8.4-6"/>'],
        ],
